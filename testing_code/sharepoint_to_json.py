        ...
        if "folder" not in file: 
            if file.get("id") not in file_id_list: 
                ...
                with open(save_to_path, "wb") as f:
                    ...
                    json_file_name=process_downloaded_files(temp_dir)
                    print({"property" : property_name, "doc_type" : parent_folder , "name" : json_file_name})
...
def convert_docx_to_pdf_and_pdf_to_json(docx_file):
    try:
        ...
        logging.error(f"Error converting DOCX to PDF and processing '{docx_file}': {str(e)}")
        traceback.print_exc()
